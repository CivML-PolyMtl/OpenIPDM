...
for t=0:T-1
    if t==0
        ...
        [ExpectedCond, Variance] = relu_constrain_step(ExpectedCond(:,t+1),Variance(:,:,t+1),constrain_vector, [-inf, -100, -inf, -inf, -inf, -inf],[+inf, 0, +inf, +inf, +inf, +inf], false);
    else
    ...
    if t~=0
        if ~isnan(y(t+1))
            ExpectedCond(1:6,t+1)=UpdateExpected(A*ExpectedCond(:,t),F*(A*ExpectedCond(:,t)),sigmaXY,S,y(t+1),B(t+1)); 
            Variance(1:6,1:6,t+1)=UpdateVariance(UPvariance,S,sigmaXY);
        else
            ExpectedCond(1:6,t+1)=A*ExpectedCond(:,t);
            Variance(1:6,1:6,t+1)=UPvariance;
        end
        ...
        SigmaForecast(1:6,t+1)=sqrt(diag(Variance(1:6,1:6,t+1)));
        SigmaForecast(2,t+1)=sqrt(Variance(7,7,t+1));
        if ~isnan(y(t+1))
