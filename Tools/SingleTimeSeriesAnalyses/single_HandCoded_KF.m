    ...
    if t~=0
        ...
        if length(InterventionVector)>1
            if InterventionVector(t+1) 
                ...
                [Mtrv]=single_RevSpaceTransform(3,ExpectedCond(1,t+1));
        ...
        if ConstrainedKF && ExpectedCond(2,t+1)+2*sqrt(Variance(2,2,t+1))>0
            ...
            [ExpectedCond(:,t+1),Variance(:,:,t+1)]=single_KFConstraintsHandlingSeq(ExpectedCond(:,t+1),Variance(:,:,t+1),D,d,1);
        end
...
for i=TotalTimeSteps-1:-1:Ending
    ...
    if ~isempty(ViIndex) && ConstrainedKF && KFCase==1
        ...
        [ExSmooth(:,i),VarSmooth(:,:,i)]=single_KFConstraintsHandlingSeq(ExSmooth(:,i),VarSmooth(:,:,i),D,d,1);
    end
end
end