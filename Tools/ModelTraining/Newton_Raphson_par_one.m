...
%              states
%%
function [MLE,S_laplace,x_NR,fx_NR]=Newton_Raphson_par_one(fx,x0,varargin)
args = varargin;
nargs = length(args);
%% Default values
...
delta = 0.001;                %Relative delta x for numeric derivatives
gamma = 0.9;                %Momentum
...
while any(hessian_NR>0)||isempty(fx_new)||(fx_NR(end)-fx_NR(end-1))>convergence_tol
    ...
    for d=1:D
        ...
        Term1_par=Term1(x_old,idx);
        Term2_par=Term2(x_old,idx);
        grad_old=(Term1_par-Term2_par)/(2*Term3(x_old,idx));          %compute gradient
        hessian_old=(Term1_par-2*fx_old+Term2_par)/(Term3(x_old,idx)^2);
    ...
    loop=100*nb_failed_iteration_limit+1;
