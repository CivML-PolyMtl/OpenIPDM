...
for i=1:length(x(1,:))
    xtb(1,i)=RevSpaceTransform(Pn,x(1,i));                                  % Back-transform the condition (Model)
    yOr(i)=RevSpaceTransform(Pn,y(i));                                      % Back-transform the condition (Observations)
    ...
    MVv(1,i)=(RevSpaceTransform(Pn,2*MVv(1,i)+x(1,i))...                    % E[x(1)]+2*sigma - E[x(1)]
        -RevSpaceTransform(Pn,x(1,i)));
    MVv(4,i)=(RevSpaceTransform(Pn,x(1,i))-...                              % E[x(1)] - (E[x]-2*sigma)
        RevSpaceTransform(Pn,x(1,i)-2*s_Xsmooth(1,i)));
    MVv(5,i)=(RevSpaceTransform(Pn,1*s_Xsmooth(1,i)+x(1,i))-...             % E[x(1)]+1*sigma - E[x(1)]
        RevSpaceTransform(Pn,x(1,i)));
    MVv(6,i)=(RevSpaceTransform(Pn,x(1,i))-...                              % E[x(1)] - (E[x(1)]-1*sigma)
        RevSpaceTransform(Pn,x(1,i)-1*s_Xsmooth(1,i)));
    ReOr(i)=(RevSpaceTransform(Pn,2*sqrt(Re(i))+y(i))...                    % E[y]+2*sigma - (E[y]-2*sigma)
        -RevSpaceTransform(Pn,y(i)-2*sqrt(Re(i))));
    Rtop(i)=RevSpaceTransform(Pn,2*sqrt(Re(i))+y(i))...                     % E[y]+2*sigma - E[y]
        -RevSpaceTransform(Pn,y(i));
    Rlow(i)=RevSpaceTransform(Pn,y(i))-RevSpaceTransform(Pn,y(i)...         % E[y] - (E[y]-2*sigma)
        -2*sqrt(Re(i)));
    ...
    if ~isempty(ReTrue)
        ...
        MVvTrue(1,i)=(RevSpaceTransform(Pn,2*MVvTrue(1,i)+...               % E[x(1)] + 2*sigma - E[x(1)] (True parameters)
            ExsmoothTrue(1,i))-RevSpaceTransform(Pn,ExsmoothTrue(1,i)));
        MVvTrue(4,i)=(RevSpaceTransform(Pn,ExsmoothTrue(1,i))-...           % E[x(1)] - (E[x]-2*sigma) (True parameters)
            RevSpaceTransform(Pn,ExsmoothTrue(1,i)-2*s_XsmoothTrue(1,i)));
        ...
        XTrueParam(i)=RevSpaceTransform(Pn,ExsmoothTrue(1,i));              % The condition (Model with True Params.) (Original Space)
        RtopTrue(i)=RevSpaceTransform(Pn,2*sqrt(ReTrue(i))+y(i))...         % E[y] + 2*sigma - E[y]
            -RevSpaceTransform(Pn,y(i));
        RlowTrue(i)=RevSpaceTransform(Pn,y(i))-...                          % E[y] - (E[y]-2*sigma)
            RevSpaceTransform(Pn,y(i)-2*sqrt(ReTrue(i)));
