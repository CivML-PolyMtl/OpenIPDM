...
% NormObsValues = RevSpaceTransform(Pn,NormObsValues,100,25);
...
%     T_0=cputime;
    ...
    if args{6}<=1
        ...
        [x, Var, ~, ~,~,~] = kalman_filter(y, A, C, Q, R, Re...
            ...
            ,Ncurve,OptBoundsData,GlobalCondData(3,1),GlobalCondData,GPUCompute,Nsigma);
        TrainSmootherRun();      
        if GPUCompute
        ...
        try
            [x, Var, ~, LogLikValidation,~,~] = kalman_filter(y_valid, A, C, Q, R_valid, Re_Valid...
                ...
                GlobalCondData(3,1),GlobalCondData,GPUCompute,Nsigma);
            TrainSmootherRun();          
        catch
            disp(' ')
            disp('Constraints Failiur')
