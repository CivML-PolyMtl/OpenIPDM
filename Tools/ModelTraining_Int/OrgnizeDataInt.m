function [MdataEngy]=OrgnizeDataInt(MdataEngy,CurrentInspectorID,...
    CurrentInspectorParam,EngBiasData,DataTransformedSpace,Ncurve,...
    OptBoundsData,AW,GPUCompute,IncludeStructuralAtt,NumStructuralAtt)
rng(6);
R=0;IndexIt=[];jj=1;
CID=sum(~cellfun(@isempty,MdataEngy),2);
TrainSize=sum(CID);
TestingData=round(0*TrainSize);
IncludeInterventions=1;
if GPUCompute==2
