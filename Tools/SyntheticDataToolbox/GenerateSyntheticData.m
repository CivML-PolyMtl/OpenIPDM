function [TrueState,ObservedData,GeneratedSyntheticData]=...
    GenerateSyntheticData(app,T,dt,DatasetSize,NumInsp,InspMinStd,...
...
if InterventionsCheck
    ...
    Cond_Percent=[0.1 0.3 0.6]; LowestCond=50; MidCond=70;% low end
else
    Cond_Percent=[0.3 0.6 0.1]; LowestCond=70; MidCond=80;% low end
end
...
%     InterventionTime_0=randi([15 19],DatasetSize,1);
    ...
    Priority=3.*rand(1,DatasetSize);%randi([0 2],DatasetSize,1)+0.5;               
...
for i=1:DatasetSize
    % initial speed in perfect condition
    ...
    % initial acceleration in perfect condition
    ...
    % perfect condition in the transformed space
    [~,Ms]=SpaceTransformation(Ncurve,MaxStart,MaxCond,MinCond);
    
    x0=[Ms;Ss;As];                             % random start state
    LowerLimit=-4;
    x(:,1)=x0;
    x(:,60)=LowerLimit;
    ...
    while RejectTimeSeries
        ...
        WeightedProbInd=1-[repmat(Count90_100/Initial90_100,1,SplitWindow)...
            ...
            repmat(Count70_80/Initial70_80,1,(MaxTimeCut-MinTimeCut+1)-2*SplitWindow)];
...
GeneratedSyntheticData.Bias=[MaxBias,MinBias];
