...
function [xTrunc,PTrunc,Success]=KFConstraintsHandlingSeq(xTrunc,PTrunc,D,d,param)
try
Success=1;
k=1;
try
...
PTrunc = Ttrunc * sqrt(Wtrunc) * S' * CovZ * S * sqrt(Wtrunc) * Ttrunc';
    ...
    sprintf('Truncation method has failed initialilly, something is wrong with the variance!')
end
end